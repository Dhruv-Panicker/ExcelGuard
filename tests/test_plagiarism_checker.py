...
from algorithim.plagiarism_checker import get_fingerprint_data, get_column_width_data, get_author_data, get_font_data, get_formula_data
...
def test_get_column_width_data(test_client):
    with app.app_context(): 
        scan_id = 56  # Example scan_id
        data = get_column_width_data(scan_id)
        assert isinstance(data, dict)  # Ensure it returns a dictionary
