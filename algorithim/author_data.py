...
def check_author_data(author_data, db, ExcelFile, template_author_data):
  ...
  for file1, file2 in combinations(author_data.items(), 2):
    ...
    if author_data1["creator"] == author_data2["creator"]:
      suspicious_files.setdefault(file_id1, []).append(("author_data", f"Same creator:{author_data1['creator']} as {file2_name}", 3))
      suspicious_files.setdefault(file_id2, []).append(("author_data", f"Same creator:{author_data2['creator']} as {file1_name}", 3))

    if author_data1["lastModifiedBy"] == author_data2["lastModifiedBy"]:
      suspicious_files.setdefault(file_id1, []).append(("author_data", f"Same last modified by:{author_data1['lastModifiedBy']} as {file2_name}", 2))
      suspicious_files.setdefault(file_id2, []).append(("author_data", f"Same last modified by:{author_data2['lastModifiedBy']} as {file1_name}", 2))
    ...
    if author_data1["created"] == author_data2["created"]:
      ...
      suspicious_files.setdefault(file_id1, []).append(("author_data", f"Same creation date:{auth1_str} as {file2_name}", 2))
      suspicious_files.setdefault(file_id2, []).append(("author_data", f"Same creation date:{auth2_str} as {file1_name}", 2))
